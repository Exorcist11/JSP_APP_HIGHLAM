document.addEventListener("DOMContentLoaded", function () {
  ...
  cart.forEach((product) => {
    ...
    row.innerHTML = `
          ...
          <td>${product.productName} - Màu ${product.selectedColor} - Size ${product.selectedSize
      }</td>
          <td>${productPrice.toLocaleString()}₫</td>
          <td>
              <div class="d-flex align-items-center">
                  <button class="btn btn-outline-secondary btn-sm decrement" data-id="${product.productId
      }">-</button>
                  <input min="1" class="form-control text-center mx-2 quantity" style="width: 50px" type="text" value="${product.quantity
      }" />
                  <button class="btn btn-outline-secondary btn-sm increment" data-id="${product.productId
      }">+</button>
              </div>
          </td>
      `;
